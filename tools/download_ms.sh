...
# Install modelscope if not already installed (OPTIONAL)
...
for round in $(seq 0 $((rounds - 1))); do
    # 1. meta data
    modelscope download --dataset ${repo} SimScale_data/synthetic_reaction_pdm_v1.0-${round}/simscale_pdm_v1.0-${round}_meta_datas.tar.gz --local_dir ./
    ...
    for idx in $(seq 0 $((split - 1))); do
        modelscope download --dataset ${repo} SimScale_data/synthetic_reaction_pdm_v1.0-${round}/simscale_pdm_v1.0-${round}_sensor_blobs_hist/simscale_pdm_v1.0-${round}_sensor_blobs_hist_${idx}.tar.gz --local_dir ./
...
for round in $(seq 0 $((rounds - 1))); do
    # 1. meta data
    modelscope download --dataset ${repo} SimScale_data/synthetic_reaction_recovery_v1.0-${round}/simscale_recovery_v1.0-${round}_meta_datas.tar.gz --local_dir ./
    ...
    split=${splits[$round]}
    for idx in $(seq 0 $((split - 1))); do
        modelscope download --dataset ${repo} SimScale_data/synthetic_reaction_recovery_v1.0-${round}/simscale_recovery_v1.0-${round}_sensor_blobs_hist/simscale_recovery_v1.0-${round}_sensor_blobs_hist_${idx}.tar.gz --local_dir ./
